 ...
 */
const getAllVocabs = async (user_id) => {
  try {
...
const exportToCSV = (vocabs) => {
  ...
  // CSV headers
  const headers = ["Vocabs", "Definitions & Examples", "Date Added"];
 ...
 */
const exportAllVocabs = async (user_id, format = "csv") => {
  try {
    console.log(`🔄 Exporting all vocabs as ${format.toUpperCase()}...`);

    const vocabs = await getAllVocabs(user_id);
